...
export default function AdminLogin() {
  const [email, setEmail] = useState("admin@yaksha.in");
  ...
  const submit = async (e) => {
    e.preventDefault();
    try {
      await signIn({ email, password, isAdmin: true });
    ...
    <div className="min-h-screen flex items-center justify-center bg-background bg-mesh px-4">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="px-7 pt-7 pb-5 text-center">
          ...
          <h1 className="font-display text-xl font-semibold">Control plane sign-in</h1>
          <p className="text-xs text-muted-foreground mt-1">Demo: any email & password — separate from student portal</p>
        </div>

        <form onSubmit={submit} className="px-7 pb-7 space-y-3">
          <Field icon={<Mail className="w-4 h-4" />} value={email} onChange={setEmail} placeholder="Admin email" type="email" required />
          <Field icon={<Lock className="w-4 h-4" />} value={password} onChange={setPassword} placeholder="Password" type="password" required />
